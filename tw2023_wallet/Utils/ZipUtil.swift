...
class ZipUtil {
    ...
    static func createZip(with content: String) -> Data? {
        ...
        do {
            ...
            // 一時ファイルをZIP圧縮
            try Zip.zipFiles(paths: [originalFilePath], zipFilePath: zipFilePath, password: nil, progress: nil)
    ...
    static func unzipAndReadContent(from zipData: Data) throws -> String {
        ...
        // 一時ZIPファイルのパスを生成
        let tempZipFilePath = tempDirectoryURL.appendingPathComponent("tempZipFile.zip")
        
        // ZIPデータを一時ファイルに書き込む
        try zipData.write(to: tempZipFilePath)
        
        // ZIPファイルを解凍
        try Zip.unzipFile(tempZipFilePath, destination: tempDirectoryURL, overwrite: true, password: nil)
        
        // 解凍したファイルの一覧を取得
        let unzippedFiles = try fileManager.contentsOfDirectory(at: tempDirectoryURL, includingPropertiesForKeys: nil, options: [])
        
        // テキストファイルが一つだけ含まれていると想定して、その内容を読み込む
        if let firstFileURL = unzippedFiles.first {
            let content = try String(contentsOf: firstFileURL, encoding: .utf8)
            return content
        } else {
            throw NSError(domain: "UnzipError", code: 0, userInfo: [NSLocalizedDescriptionKey: "ZIPファイルに含まれるテキストファイルが一つではないか、見つかりません。"])
        }
    }
}
