...
struct SDJwtUtil {
    ...
    static func divideSDJwt(_ sdJwt: String) throws -> SDJwtParts {
        ...
        guard parts.count > 0 else {
            throw NSError(domain: "com.example.error", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid SD-JWT: No parts found"])
        }
    ...
    static func decodeDisclosure(_ disclosures: [String]) -> [Disclosure] {
        return disclosures.map { d in
            let decodedString = String(data: Data(base64Encoded: base64urlToBase64(base64url: d)) ?? Data(), encoding: .utf8) ?? ""
            guard let decoded = try? JSONSerialization.jsonObject(with: decodedString.data(using: .utf8)!) as? [Any] else {
    ...
    static func getDecodedJwtHeader(_ sdJwt: String) -> [String: Any]? {
        ...
        let headerData = issuerParts?.first.flatMap { Data(base64Encoded: base64urlToBase64(base64url: $0)) }
    ...
    static func getX509CertificatesFromJwt(_ jwt: [String: Any]) -> [(String, Data)]? {
        ...
        for cert in x5cValues{
             guard let der = Data(base64Encoded: cert),
                   let pem = SignatureUtil.base64strToPem(base64str: cert) else {
                return nil
            }
