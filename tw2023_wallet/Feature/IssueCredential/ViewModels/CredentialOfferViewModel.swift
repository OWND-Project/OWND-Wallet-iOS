...
class CredentialOfferViewModel {
    ...
    func initialize(rawCredentialOfferString: String) throws {
        ...
        self.credentialOffer = try JSONSerialization.jsonObject(with: jsonData!, options: []) as? [String: Any]
    }
    
    func checkIfPinIsRequired() -> Bool {
        if let grants = credentialOffer!["grants"] as? [String: Any],
           let preAuthCodeInfo = grants["urn:ietf:params:oauth:grant-type:pre-authorized_code"] as? [String: Any],
    ...
    func sendRequest(userPin: String?) async throws {
        do {
            ...
            let vciClient = try await VCIClient(credentialOfferJson:
            ...
            if (proofRequired && cNonce != nil){
                ...
                let proofJwt = try KeyPairUtil.createProofJwt(keyAlias: Constants.Cryptography.KEY_BINDING, audience: credentialIssuer, nonce: cNonce!)
                proofObject = Proof(proofType: "jwt", jwt: proofJwt)
            }
            
            // Credential Request Generation
            let credentialRequest = createCredentialRequest(formatValue: credential_format!, vctValue: credential_vct!, proof: proofObject)
            
            // 発行
            let credentialResponse = try await vciClient.issueCredential(payload: credentialRequest, accessToken: accessToken)
            
            // 保存
            let protoBuf = convertToProtoBuf(accessToken: accessToken, credentialResponse: credentialResponse)
    ...
    private func convertToProtoBuf(accessToken: String, credentialResponse: CredentialResponse) -> Datastore_CredentialData {
        let basicInfo: [String: Any] = credential_format == "vc+sd-jwt" ?
            extractSDJwtInfo(credential: credentialResponse.credential, format: credential_format!) :
        ...
        let expiresIn = credentialResponse.cNonceExpiresIn == nil ? Int32(0) : Int32(credentialResponse.cNonceExpiresIn!)
    ...
    private func parseCredentialOfferJsonString() throws -> String {
        ...
        guard let credentialOfferValue = queryItems.first(where: { $0.name == "credential_offer" })?.value else {
    ...
    private func interpretMetadataAndOffer(){
        ...
        self.dataModel.metaData!.credentialsSupported.forEach { (key, credentialSupported) in
    ...
    private func extractInfoFromJwt(jwt: String, format: String) -> [String: Any] {
        guard let decodedPayload = jwt.components(separatedBy: ".")[1].base64UrlDecoded(),
              ...
              let jwtDictionary = try? JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] else {
            return [:]
        }
