...
func parse(uri: String) throws -> (String, AuthorizationRequestPayload) {
    guard let url = URL(string: uri), let scheme = url.scheme else {
        throw AuthorizationRequestError.authRequestInputError(reason: .compliantError(reason: "invalid url format"))
    }
    do {
        ...
        print(error)
        throw AuthorizationRequestError.authRequestInputError(reason: .compliantError(reason: "failed to parse and decode url"))
    }
}
...
func fetchJWT(from url: URL, using session: URLSession = URLSession.shared) async throws -> String {
    ...
    if (httpResponse.statusCode == 200) {
        ...
        throw AuthorizationRequestError.authRequestClientError(reason: .badRequest(reason: "request of `request object` is bad request."))
    } else if (httpResponse.statusCode == 404) {
        throw AuthorizationRequestError.authRequestInputError(reason: .resourceNotFound(reason: "request object jwt"))
    } else  {
        throw AuthorizationRequestError.unknown(reason: nil)
    }

    guard let jwtString = String(data: data, encoding: .utf8) else {
        throw AuthorizationRequestError.authRequestInputError(reason: .compliantError(reason: "invalid jwt string"))
    }

    return jwtString
}

func fetchJson(from url: URL, using session: URLSession = URLSession.shared) async throws -> [String: Any] {
    let (data, response) = try await session.data(from: url)
...
func processRequestObject(_ authorizationRequest: AuthorizationRequestPayload, using session: URLSession = URLSession.shared) async throws -> (String, RequestObjectPayload) {
    ...
    } else if let requestUriString = authorizationRequest.requestUri, let requestUri = URL(string: requestUriString) {
...
func processClientMetadata(_ authorizationRequest: AuthorizationRequestPayload, _ requestObject: RequestObjectPayload, using session: URLSession = URLSession.shared) async throws -> RPRegistrationMetadataPayload {
    
    if let clientMetadata = requestObject.clientMetadata ?? authorizationRequest.clientMetadata {
        return clientMetadata
    } else {
        let clientMetadataUri = requestObject.clientMetadataUri ?? authorizationRequest.clientMetadataUri
...
func processPresentationDefinition(_ authorizationRequest: AuthorizationRequestPayload, _ requestObject: RequestObjectPayload, using session: URLSession = URLSession.shared) async throws -> PresentationDefinition? {
    
    if let presentationDefinition = requestObject.presentationDefinition ?? authorizationRequest.presentationDefinition {
        return presentationDefinition
    } else {
        let presentationDefinitionUri = requestObject.presentationDefinitionUri ?? authorizationRequest.presentationDefinitionUri
...
func parseAndResolve(from uri: String, using session: URLSession = URLSession.shared) async -> Result<ProcessedRequestData, AuthorizationRequestError> {
    do {
        print("parse")
        let (_, authorizationRequest) = try parse(uri: uri)
        
        print("process request object")
        let (jwt, requestObject) = try await processRequestObject(authorizationRequest, using: session)
        print(requestObject)
        
        print("process client metadata")
        let clientMetadata = try await processClientMetadata(authorizationRequest, requestObject, using: session)
        
        print("process presentation definition")
        let presentationDefinition = try await processPresentationDefinition(authorizationRequest, requestObject, using: session)
...
func fetchAndConvertJWK(from url: URL, withKeyId keyId: String, using session: URLSession = URLSession.shared) async throws -> SecKey? {
...
func verifyRequestObject(jwt: String, clientMetadata: RPRegistrationMetadataPayload, using session: URLSession = URLSession.shared) async -> Result<JWT, AuthorizationRequestError> {
    ...
    guard let decoded = try? JWTUtil.decodeJwt(jwt: jwt) else {
        print(jwt)
        return .failure(.authRequestInputError(reason: .compliantError(reason: "can not decode jwt")))
    }
    let (header, _, _) = decoded
    guard let keyId = extractKeyIdFromJwt(header: header) else {
        return .failure(.authRequestInputError(reason: .compliantError(reason: "can not find kid in header")))
    }
    do {
        guard let key = try await fetchAndConvertJWK(from: URL(string: jwksUrl)!, withKeyId: keyId, using: session) else {
            return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get public key from jwks url")))
        }
        print("!!!! JWT")
        print(jwt)
        print(key)
        ...
        case .failure(let error):
            print(error)
            return .failure(.authRequestInputError(reason: .compliantError(reason: "failed to validate jwt")))
        }
    } catch {
        return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get public key from jwks url")))
    }
