...
class EncryptionHelper {
    ...
    func decrypt(data: Data, iv: Data, tag: Data) -> Data? {
        ...
        guard let sealedBox = try? AES.GCM.SealedBox(nonce: nonce, ciphertext: data, tag: tag) else { return nil }
    ...
    func deserializeEncrypted(serializedEncrypted: String) -> (encryptedData: Data, iv: Data, tag: Data)? {
        ...
        guard let encryptedData = Data(base64Encoded: parts[0]),
              ...
              let tag = Data(base64Encoded: parts[2]) else {
            return nil
        }
