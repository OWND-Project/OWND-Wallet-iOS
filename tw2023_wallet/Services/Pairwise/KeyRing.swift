...
class HDKeyRing {
    ...
    init?(mnemonicWords: String? = nil, entropyLength: Int = 128) {
        ...
        if let mnemonicWords = mnemonicWords {
            ...
            guard let seed = BIP39.seedFromMmemonics(mnemonicWords, password: "", language: .english) else {
                return nil
            }
            guard let rootNode = HDNode(seed: seed)?.derive(path: "m", derivePrivateKey: true) else {
                return nil
            }
            ...
            guard let seed = BIP39.seedFromMmemonics(mnemonics!, password: "", language: .english) else {
                return nil
            }
            guard let rootNode = HDNode(seed: seed)?.derive(path: "m", derivePrivateKey: true) else {
                return nil
