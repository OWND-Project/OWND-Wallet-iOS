...
class KeyPairUtil {
    ...
    static func createProofJwt(keyAlias: String, audience: String, nonce: String) throws -> String {
        ...
        guard let publicKey = KeyPairUtil.getPublicKey(alias: keyAlias),
              let jwk = publicKeyToJwk(publicKey: publicKey) else {
    ...
    static func createPublicKey(jwk: [String: String]) throws -> SecKey {
        ...
        guard let secKey = SecKeyCreateWithData(publicKeyData as CFData, [
            ...
            if let error = error?.takeRetainedValue() {
                ...
                throw NSError(domain: "com.example", code: 500, userInfo: [NSLocalizedDescriptionKey: "公開鍵の作成に失敗しました"])
            }
        }
    ...
    static func publicKeyToJwk(publicKey: SecKey) -> [String: String]?{
        guard let attributes = SecKeyCopyAttributes(publicKey) as? [String: Any] else {
            ...
            guard let keyType = attributes[kSecAttrKeyType as String] as? String,
                  ...
                  let _ = attributes[kSecValueData as String] as? Data else {
                return nil
            }
...
class KeyStoreHelper {
    ...
    func generateSecretKey() throws -> Data {
        ...
        guard result == errSecSuccess else {
            throw NSError(domain: "com.example", code: 500, userInfo: [NSLocalizedDescriptionKey: "暗号化キーの生成に失敗しました"])
        }
...
class KeychainWrapper {
    func getData(forKey key: String) throws -> Data? {
        ...
        if status == errSecSuccess, let data = result as? Data {
            ...
            throw NSError(domain: "com.example", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "キーチェーンからデータを取得できませんでした"])
        }
    }
    ...
    func setData(_ data: Data, forKey key: String) throws {
        ...
        if status != errSecSuccess {
            throw NSError(domain: "com.example", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "キーチェーンにデータを保存できませんでした"])
        }
    }
}
