...
class SDJwtUtilTests: XCTestCase {
    ...
    let sdJwtWithDisclosureWithoutKBJ_instance =
        ...
        do {
            ...
            for disclosure in decodedDisclosures {
                if let keyName = disclosure.key {
                    XCTAssertTrue(
                        expected.contains(keyName), "Key '\(keyName)' not found in the expected set"
                    )
                }
            }
        }
    ...
    func testgetDecodedJwtHeader() {
        guard let headerJSON = SDJwtUtil.getDecodedJwtHeader(sdJwtWithDisclosureWithoutKBJ_instance)
        else {
