...
enum SignatureUtil {
    ...
    static func generateCertificate(subjectPublicKey: P256.Signing.PublicKey, issuerPrivateKey: P256.Signing.PrivateKey, isCa: Bool) -> Certificate {
        ...
        let extensions = try! Certificate.Extensions {
            Critical(
                isCa ? BasicConstraints.isCertificateAuthority(maxPathLength: nil) : BasicConstraints.notCertificateAuthority
            )
            Critical(
    ...
    static func generateSelfSignedCertificate(issuerPrivateKey: P256.Signing.PrivateKey) -> Certificate {
        return generateCertificate(subjectPublicKey: issuerPrivateKey.publicKey, issuerPrivateKey: issuerPrivateKey, isCa: true)
    }
...
//    }
    static func validateCertificateChain(derCertificates: [Data?]) throws -> Bool {
        ...
        let certs = derCertificates.compactMap { $0 }.map {
