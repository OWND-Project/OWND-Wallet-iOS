...
final class CredentialIssuerMetadataTests: XCTestCase {
    ...
    func testFetchAuthServerMetadata() {
        runAsyncTest {
            ...
            let testURL = URL(string: "https://example.com/api/endpoint")!
            guard let url = Bundle.main.url(forResource: "authorization_server", withExtension: "json"),
                  let mockData = try? Data(contentsOf: url) else {
    ...
    func testRetrieveAllMetadata() {
        runAsyncTest {
            ...
            guard let url = Bundle.main.url(forResource: "credential_issuer_metadata_jwt_vc", withExtension: "json"),
                  let mockData1 = try? Data(contentsOf: url) else {
                XCTFail("Cannot read credential_issuer_metadata.json")
                return
            }
            let testURL2 = URL(string: "\(issuer)/.well-known/oauth-authorization-server")!
            guard let url = Bundle.main.url(forResource: "authorization_server", withExtension: "json"),
                  let mockData2 = try? Data(contentsOf: url) else {
                XCTFail("Cannot read authorization_server.json")
                return
            }
            let response = HTTPURLResponse(url: url, statusCode: 200, httpVersion: nil, headerFields: nil)
