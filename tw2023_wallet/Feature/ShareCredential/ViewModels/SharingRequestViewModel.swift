...
class SharingRequestViewModel {
    ...
    func loadData(_ url: String, index: Int = -1) async {
        ...
        do {
            ...
            case .success(let processedRequestData):
                ...
                guard let url = URL(string: clientId), let schema = url.scheme, let host = url.host else {
            ...
            case .failure(let error):
                ...
                case .authRequestClientError(let subError):
                    ...
                    case .badRequest(let reason):
                        alertTitle = "Sent Wrong request. It needs to confirm the request sent."
                        alertMessage = reason
                    case .compliantError(let reason):
                        alertTitle = "Client error occurred. It needs to confirm wallet app."
                        alertMessage = reason
                    }
    ...
    func shareIdToken() async -> Result<PostResult, Error> {
        ...
        guard let openIdProvider = openIdProvider,
              let account = account,
              let seed = seed,
              let accountManager = PairwiseAccount(mnemonicWords: seed) else {
            let errorState =
                openIdProvider == nil ? ShareIdTokenError.illegalOpenIdProviderState :
                account == nil ? ShareIdTokenError.illegalAccountState :
                seed == nil ? ShareIdTokenError.illegalSeedState :
