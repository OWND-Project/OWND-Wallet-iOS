...
enum OpenIdProviderRequestException: Error {
    case badAuthRequest
    case unavailableAuthRequest // already consumed request, failed to get client additinal info(request jwt, client metada, etc), etc
...
class OpenIdProvider {
    ...
    func processSIOPRequest(_ url: String, using session: URLSession = URLSession.shared) async -> Result<ProcessedRequestData, AuthorizationRequestError> {
        ...
        case .success(let processedRequestData):
            ...
            guard let _clientId = requestObj.clientId ?? authRequest.clientId else {
                return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get client id")))
            }
            clientId = _clientId
            ...
            if (clientScheme == "redirect_uri") {
                ...
                if (clientId != responseUri) {
                    return .failure(.authRequestInputError(reason: .compliantError(reason: "Invalid client_id or response_uri")))
                }
            }
            
            guard let responseType = requestObj.responseType ?? authRequest.responseType else {
                return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get response type")))
            }
            ...
            guard let _nonce = requestObj.nonce ?? authRequest.nonce else {
                return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get nonce")))
            }
            nonce = _nonce
            state = requestObj.state ?? authRequest.state ?? ""
            if (responseType.contains("vp_token")) {
                guard let _presentationDefinition = processedRequestData.presentationDefinition else {
                    return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get presentation definition")))
                }
                presentationDefinition = _presentationDefinition
            }
            guard let _redirectUri = requestObj.redirectUri ?? authRequest.requestUri else {
                return .failure(.authRequestInputError(reason: .compliantError(reason: "can not get redirect uri")))
            }
...
func respondSIOPResponse(using session: URLSession = URLSession.shared) async -> Result<PostResult, Error> {
        guard let authRequestProcessedData = self.authRequestProcessedData else {
            return .failure(OpenIdProviderIllegalStateException.illegalAuthRequestProcessedDataState)
        }
    ...
    func convertIdTokenResponseResponse(data: Data, response: HTTPURLResponse, requestURL: URL) throws -> PostResult {
...
//                    location = base.absoluteString + locationHeader
                ...
                return PostResult(statusCode: response.statusCode, location: location, cookies: cookies)
            } else {
    ...
    func convertVpTokenResponseResponse(data: Data, response: HTTPURLResponse, requestURL: URL) throws -> PostResult {
    ...
    func respondVPResponse(credentials: [SubmissionCredential], using session: URLSession = URLSession.shared) async -> Result<(PostResult, [SharedContent], [String?]), Error> {
//        guard let authRequestProcessedData = self.authRequestProcessedData else {
...
//        let requestObj = authRequestProcessedData.requestObject
        guard let clientId = clientId,
              ...
              let responseUri = authRequestProcessedData?.requestObject.responseUri else {
        ...
        if vpTokens.count == 1 {
            ...
            if let jsonData = try? jsonEncoder.encode(tokens), let jsonString = String(data: jsonData, encoding: .utf8) {
    ...
    func createPresentationSubmissionSdJwtVc(
        ...
        guard let (inputDescriptor, selectedDisclosures) = selectDisclosure(sdJwt: sdJwt, presentationDefinition: presentationDefinition) else {
            // TODO: エラーハンドリングかダミーの戻り値
            return ("Dummy", DescriptorMap(id: "dummyId", format: "dummyFormat", path: "dummyPath", pathNested: Path(format: "dummyFormat", path: "dummyPath")), [DisclosedClaim](), "dummyPurpose")
        }
        ...
        let keyBindingJwt = try keyBinding.generateJwt(sdJwt: sdJwt, selectedDisclosures: selectedDisclosures, aud: clientId, nonce: nonce)
                
        let parts = sdJwt.split(separator: "~").map(String.init)
        guard let issuerSignedJwt = parts.first else {
            return ("Error", DescriptorMap(id: "error", format: "error", path: "error", pathNested: Path(format: "error", path: "error")), [DisclosedClaim](), "dummyPurpose")
        }
        ...
        let vpToken = issuerSignedJwt + "~" + selectedDisclosures.map { $0.disclosure! }.joined(separator: "~") + "~" + keyBindingJwt
        ...
        let disclosedClaims = selectedDisclosures.compactMap { disclosure -> DisclosedClaim? in
            ...
            return DisclosedClaim(id: credential.id, types: credential.types, name: key, value: disclosure.value)
        }
...
func satisfyConstrains(credential: [String: Any], presentationDefinition: PresentationDefinition) -> Bool {
...
func selectDisclosure(sdJwt: String, presentationDefinition: PresentationDefinition) -> (InputDescriptor, [Disclosure])? {
    let parts = sdJwt.split(separator: "~").map(String.init)
    ...
    for inputDescriptor in presentationDefinition.inputDescriptors {
        let matchingDisclosures: [Disclosure] = presentationDefinition.inputDescriptors.compactMap { inputDescriptor in
            /*
