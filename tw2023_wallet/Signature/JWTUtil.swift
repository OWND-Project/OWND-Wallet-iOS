...
enum JWTUtil {
    ...
    static func sign(keyAlias: String, header: [String: Any], payload: [String: Any]) throws -> String {
        guard let privateKey = KeyPairUtil.getPrivateKey(alias: keyAlias) else {
            throw KeyError.KeyNotFound
        }
        
        guard let h = try? JWTUtil.jsonString(from: header).data(using: .utf8)?.base64URLEncodedString(),
              let p = try? JWTUtil.jsonString(from: payload).data(using: .utf8)?.base64URLEncodedString() else {
        ...
        guard let firstElm = sequence.sub(0)?.value as? Data,
              let secondElm = sequence.sub(1)?.value as? Data else {
    ...
    static func verifyJwt(jwt: String, publicKey: SecKey) -> Result<JWT, JWTVerificationError> {
        ...
        let result = SecKeyVerifySignature(publicKey, signatureAlg, tbsContent as CFData, derSignature as CFData, nil)
        if (result) {
            return .success(decodedJwt)
        } else {
            return .failure(JWTVerificationError.verificationFailed("result of SecKeyVerifySignature is false"))
        }
    }
    ...
    static func verifyJwtByX5U(jwt: String) -> Result<JWT, JWTVerificationError> {
        ...
        var error: Unmanaged<CFError>?
        guard let secKey = SecKeyCreateWithData(publicKeyCFData, [
    ...
    static func decodeJwt(jwt: String) throws -> ([String: Any], [String: Any], String) {
        ...
        if parts.count != 3 {
            throw NSError(domain: "InvalidJWTFormatError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid JWT format"])
        }
    ...
    static func jsonToMap(json: String) throws -> [String: Any] {
        ...
        guard let dictionary = jsonObject as? [String: Any] else {
            throw NSError(domain: "ParsingError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to parse JSON"])
        }
