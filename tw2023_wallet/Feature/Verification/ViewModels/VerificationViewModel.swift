...
class VerificationViewModel {
    ...
    func setDisplayMap(displayMapString: String) {
        displayMap = VCIMetadataUtil.deserializeDisplayByClaimMap(
        ...
        if let jsonString = decompressed, let data = jsonString.data(using: .utf8) {
            do {
                ...
                let decodedData = try JSONDecoder().decode(
                    DecompressedVerificationArgs.self, from: data)
                    ...
                    let display = decodedData.display
                {
                    ...
                    self.displayMap = VCIMetadataUtil.deserializeDisplayByClaimMap(
                        displayMapString: display)
                }
            }
            catch {
                ...
                if format == "jwt_vc_json" {
                        ...
                        let credentialSubject = vc["credentialSubject"] as? [String: String]
                    {
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.dataModel.claims = self.transformCredentialSubjectIntoClaims(
                                credentialSubject)
    ...
    func transformCredentialSubjectIntoClaims(_ credentialSubject: [String: String]) -> [(
        String, String
    )] {
